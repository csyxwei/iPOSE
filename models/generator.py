...
class OASIS_Generator(nn.Module):
    def __init__(self, opt):
        ...
        input_dim = self.opt.semantic_nc + opt.part_nc
    ...
    def forward(self, input, z=None):
        ...
        if type(input) is tuple:
            seg = torch.cat((input[0], input[1]), 1)
        else:
            seg = input
        ...
        if self.opt.use_coord:
            ...
            y = y.expand([seg.shape[0], 1, -1, -1])
            x = x.expand([seg.shape[0], 1, -1, -1])
            coord_feat = torch.cat([x, y], 1)
            seg = torch.cat((seg, coord_feat), 1)
