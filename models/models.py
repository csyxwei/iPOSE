...
class OASIS_model(nn.Module):
    ...
    def forward(self, image, label_in, mode, losses_computer):
        # Branching is applied to be compatible with DataParallel
        if type(label_in) is tuple:
            label = label_in[0]
        else:
            label = label_in
    ...
    def load_checkpoints(self):
        if self.opt.phase == "test":
            ...
            self.netG.load_state_dict(torch.load(path + "G.pth"))
            #
            now_state_dict = self.netD.state_dict()
            old_state = torch.load(path + "D.pth")

            for k in now_state_dict:
                if not k in old_state:
                    continue
                now_state_dict[k] = old_state[k]
            self.netD.load_state_dict(now_state_dict)
