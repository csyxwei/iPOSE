...
class fid_pytorch():
    ...
    def compute_fid_with_valid_path(self, model, model_):
        pool, logits, labels = [], [], []
        self.model_inc.eval()
        model.eval()
        with torch.no_grad():
            for i, data_i in enumerate(self.val_dataloader):
                image, label = model_.preprocess_input(self.opt, data_i)
    ...
    def update(self, model, cur_iter, model_):
        print("--- Iter %s: computing FID ---" % (cur_iter))
        cur_fid = self.compute_fid_with_valid_path(model, model_)
